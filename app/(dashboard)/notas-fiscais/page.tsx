...
export default function NotasFiscais() {
  ...
  return (
    <>
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-slate-800">Notas Fiscais</h2>
        <p className="text-slate-500 mt-1 text-sm">Registro e controle de notas fiscais emitidas</p>
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
          <div>
            ...
            <select className="appearance-none block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm transition-all shadow-sm" value={receitaId} onChange={(e) => setReceitaId(e.target.value)}>
          ...
          <div>
            ...
            <input type="date" className="appearance-none block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-sm transition-all shadow-sm" value={dataEmissao} onChange={(e) => setDataEmissao(e.target.value)} />
          </div>
          <button onClick={salvarNota} className="w-full bg-blue-600 hover:bg-blue-700 text-white px-6 py-2.5 rounded-xl transition-all font-bold text-sm h-[42px] border border-blue-700 shadow-sm flex items-center justify-center shrink-0 active:scale-95">
            Registrar NF
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex items-center gap-2">
          ...
          <h3 className="text-lg font-bold text-slate-800">Notas Emitidas</h3>
        </div>
        ...
        {carregando ? (
          ...
          <div className="text-center py-16 px-4">
            ...
            <h3 className="text-lg font-bold text-slate-800 mb-1">Nenhuma nota fiscal registrada</h3>
            <p className="text-slate-500 text-sm">Registre a primeira nota acima.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100">
                {notas.map((n) => (
                  <tr key={n.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap font-bold text-slate-800 text-sm">{n.numero_nota}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{n.receitas?.descricao || '—'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{n.data_emissao ? new Date(n.data_emissao + 'T00:00:00').toLocaleDateString('pt-BR') : '—'}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-right font-bold text-emerald-600">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(n.valor))}</td>
                  </tr>
                ))}
              </tbody>
            </table>
